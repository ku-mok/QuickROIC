...
class TestTranformDfDict(unittest.TestCase):
    ...
    def test_unaltered_after_df_to_dict_df_int_float(self):
        # floatとintが混在している場合の変換が逆変換になっているか
        input = pd.DataFrame.from_dict(
            {'企業名称': ['Sample1']*3 + ['Sample2']*3 + ['Sample3']*3,
             '年度': [2014, 2015, 2016]*3,
             '指標1': [1, 3, 5, 7, 9, 11, 13, 15, 17],
    ...
    def test_unaltered_after_df_to_dict_df_int_str(self):
        # strとnumericが混在している場合の変換が逆変換になっているか
        input = pd.DataFrame.from_dict(
            {'企業名称': ['Sample1']*3 + ['Sample2']*3 + ['Sample3']*3,
             '年度': [2014, 2015, 2016]*3,
             '指標1': [1, 3, 5, 7, 9, 11, 13, 15, 17],
